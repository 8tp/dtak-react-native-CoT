...
for (const fixturename of await fs.readdir(new URL('./fixtures/', import.meta.url))) {
    describe('Protobuf Reversal Tests', () => {
        test(`Protobuf Reversal Tests: ${fixturename}`, async () => {
            ...
            expect(await CoTParser.to_geojson(output)).toEqual(fixture);
        });
    });
}
...
describe('Protobuf Tests', () => {
    ...
    test('Protobuf Multiple Calls', async () => {
        ...
        expect(cot2.raw.event.detail?.contact?._attributes.callsign).toBe('sign')
        const cot3 = await CoTParser.from_proto(await CoTParser.to_proto(cot))
        expect(cot3.raw.event.detail?.contact?._attributes.callsign).toBe('sign')
    });
});
