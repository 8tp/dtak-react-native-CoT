...
describe('Shape CoT Tests', () => {
    test('En-decode polyline', async () => {
        ...
        expect(Array.isArray(vertex) && vertex.length === 5).toBe(true);
    });
    ...
    test('En-decode 0-length polyline', async () => {
        ...
        expect(cot4.raw.event.detail?.shape?.polyline).toBeDefined();
    });
    ...
    test('Basic Circle', async () => {
        ...
        if (!parsedCoT.raw.event.detail) {
            ...
            expect(parsedCoT.raw.event.detail['_flow-tags_']).toBeDefined();
