...
describe('UAS Tool CoT Tests', () => {
    test('Sensor2Polygon', async () => {
        ...
        if (!cot.raw.event.detail) {
            ...
            // Round both actual and expected polygons to avoid failing on floating point noise
            const round = (n: number) => Number(n.toFixed(12));
            const roundPolygon = (poly: any) => ({
                type: poly.type,
                coordinates: poly.coordinates.map((ring: any) => ring.map((pt: any) => [round(pt[0]), round(pt[1])]))
            });
            ...
            expect(roundPolygon(cot.sensor())).toEqual(roundPolygon(expected));
        }
    });
    ...
    test('Decode iTAK COT UAS Message', async () => {
        ...
        if (!cot.raw.event.detail) {
            ...
            expect(await CoTParser.to_geojson(cot)).toEqual({
                id: '2983J8B001V013',
                type: 'Feature',
                path: '/',
                properties: {
